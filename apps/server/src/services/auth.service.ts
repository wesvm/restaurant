...
export const loginWithPassword = async (data: AdminLoginInput): Promise<AuthResponse> => {
  const [user] = await db.select().from(users).where(eq(users.username, data.username))

  if (!user) {
    throw new AppError(401, 'Nombre de usuario incorrecto')
  }
  ...
  if (!user.password) {
    throw new AppError(400, 'Usuario sin contraseña configurada')
  }
